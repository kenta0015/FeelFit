// Full updated version using Supabase only
...
import { supabase } from '../../lib/supabase'; // または '../../../lib/supabase' に調整
...
export default function HomeScreen() {
  ...
  const handleWorkoutSelect = (workoutId: string) => {
    ...
    const session: WorkoutSession = {
      ...
      startTime: new Date().toISOString(), // ← Supabaseはstring形式を期待
  ...
  const handleWorkoutComplete = async (completionPercentage: number) => {
    ...
    if (improvementType === 'mental' && selectedEmotion) {
      ...
      mentalStaminaGain = calculateMentalStaminaGain(selectedExercise.duration, selectedExercise.intensity, effectiveness, completionPercentage);
      ...
      physicalStaminaGain = calculatePhysicalStaminaGain(selectedExercise.duration, selectedExercise.intensity, effectiveness, completionPercentage);
      ...
      mentalStaminaGain = calculateMentalStaminaGain(selectedExercise.duration, selectedExercise.intensity, mentalEffectiveness, completionPercentage);
      physicalStaminaGain = calculatePhysicalStaminaGain(selectedExercise.duration, selectedExercise.intensity, physicalEffectiveness, completionPercentage);
    }
  ...
  const goBack = () => {
    if (currentStep === 'workout-execution') {
      ...
      setCurrentStep(improvementType === 'both' ? 'physical-purpose' : improvementType === 'mental' ? 'emotion' : 'physical-purpose');
      setSelectedTime(null);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        {currentStep === 'improvement-type' && <ImprovementTypeSelector onTypeSelect={handleImprovementTypeSelect} />}
        {currentStep === 'emotion' && <EmotionSelector selectedEmotion={selectedEmotion} onEmotionSelect={handleEmotionSelect} />}
        {currentStep === 'physical-purpose' && (
          <PhysicalPurposeSelector selectedPurpose={selectedPhysicalPurpose} onPurposeSelect={handlePhysicalPurposeSelect} />
        )}
        {currentStep === 'time' && <TimeSelector selectedTime={selectedTime} onTimeSelect={handleTimeSelect} />}
        {currentStep === 'workouts' && <WorkoutList workouts={getWorkoutMatches()} onWorkoutSelect={handleWorkoutSelect} />}
