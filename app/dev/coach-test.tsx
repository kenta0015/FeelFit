...
export default function CoachTest() {
  ...
  const setTracksFromPick = () => {
    ...
    mixer.setTracks({
      tracks: picks.map((t) => t.uri), // string[]
      shuffle,
    });
  };
    ...
    <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
      ...
      >
        ...
        <Text style={{ color: "#4B5563", marginBottom: 10 }}>
          Ducking: {mixer.state.ducking ? "ON" : "OFF"} | BPM Tier: {tier}{" "}
          <Text style={{ color: "#9CA3AF" }}>(Next: {Math.min(150, tier + 20)})</Text>
        </Text>
        ...
        <Btn
          title={intents.coachBusy ? "🎤 Speaking…" : "🎤 PTT: Hold to ask"}
          onPress={togglePTT}
          disabled={false}
        />
      </View>
      ...
      >
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 4 }}>
          ...
          <Btn
            title={mixer.state.isPlayingA ? "Stop" : "Play"}
            onPress={() => (mixer.state.isPlayingA ? mixer.stop() : mixer.playHealing())} // start→playHealing
          />
